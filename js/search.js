  ...
  for (i = 0; i < this.searchlist.length; i++) {
    if (
      ...
      this.searchlist[i]
        ...
        .toUpperCase() === e.target.value.replace(/\s/gi, "").toUpperCase()
    ) {
      var count = 0;
      ...
      b.innerHTML =
        "<strong>" +
        this.searchlist[i].substr(0, e.target.selectionEnd + count) +
        "</strong>";
...
Search.prototype.searchtrantition = function (index) {
  ...
  setTimeout(
    function () {
      ...
      var initx =
        -(size / this.rangegrid) *
        (searchlist.length > this.grindcount
          ? this.grindcount - 1
          : searchlist.length - 1);
      var initz =
        -(size / this.rangegrid) *
        Math.floor((searchlist.length - 1) / this.grindcount);
