...
Popup.prototype.showimage = function (cellarray, cellIdx) {
  ...
  const close_btn = document.querySelector(".mdal_close");
  ...
  for (var i = 0; i < data.centroids.centroids.length; i++) {
    ...
    if (this.index !== -1) {
      ...
      this.currentIdx = this.cluster_group[this.selectedindex];
  ...
  if (data.cells[cellIdx]?.isVideo === 1) {
    ...
    image.onload = function () {
      ...
      setTimeout(() => {
        ...
        if(!this.result.has(this.resultCount)) {
          this.result.set(this.resultCount++, { fps: this.fpsArr, ms: this.msArr})
        }
...
Popup.prototype.createImagSubElem = function (cellIdx) {
  ...
  subimage.addEventListener("click", (e) => {
    ...
    /**
     * subImageList가 순차적이지 않음
     * 이거 cellIdx 차이만큼 prev()/next()를 호출하는게 더 좋아보임;;;
     * 
     * 아니라면 cellIdx로 selectedindex를 찾는 방식으로 가야하지만 일단 보류
     * 기능 살릴지도 모르는 상황임
     */
    if (this.cellIdx > parseInt(e.target.classList[1].split("_")[1])) {
      this.selectedindex -=
        this.cellIdx - parseInt(e.target.classList[1].split("_")[1]);
    } else {
      this.selectedindex +=
        parseInt(e.target.classList[1].split("_")[1]) - this.cellIdx;
    }

    //
    this.currentIdx = parseInt(e.target.classList[1].split("_")[1]);
...
Popup.prototype.addEventlisteners = function () {
  ...
  // document
  //   .getElementById("cb2")
  //   .addEventListener("change", this.onChnage.bind(this));
  // document
  //   .querySelector(".popup_button")
  //   .addEventListener("click", this.imagetransfer.bind(this));
  // document
  //   .getElementById("bt_clear")
    ...
    xhr.onload = function () {
      if (xhr.status === 200 || xhr.status === 201) {
        ...
        for (let i = 0; i < this.data.atlasloc.length; i++) {
          if (
            label === this.data.atlasloc[i].sub_path.split("@")[1].split("/")[0]
          ) {
...
Popup.prototype.setSlideShow = function () {
  ...
  //let cal = -(width+10)
  document.querySelector(".slide_").style.transform =
    "translateX(" + cal + "px)";
};
...
Popup.prototype.setDetail = function (fileinfodata) {
  ...
  if (detailInfo.dataMovieUrl === "") {
    ...
    document.querySelector(".t4").innerHTML =
      detailInfo.dataDesc + fileinfodata.detailInfo.dataSubpathName;
  } else {
